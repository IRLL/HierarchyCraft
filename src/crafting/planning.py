...
class CraftingPlanningProblem:
    """Interface between the unified planning framework and Crafting."""

    def __init__(
        self, state: "CraftingState", name: str, purpose: Optional["Purpose"]
    ) -> None:
        """Initialize a Crafting planning problem on the given state and purpose.

        Args:
            state: Initial state of the Crafting environment.
    ...
    def action_from_plan(self, state: "CraftingState") -> int:
        ...
        Args:
            state (CraftingState): Current state of the crafting environement.
    ...
    def update_problem_to_state(self, state: "CraftingState"):
        """Update the planning problem initial state to the given state.

        Args:
            state: Crafting state to use as reference for the
